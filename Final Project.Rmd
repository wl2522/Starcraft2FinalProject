...
5. Visualize these variables together with the time variables to get an imprecise idea of how these metrics change over time.
...
> Plotting the distribution of APM versus league supports the idea these outlier *APM* values were artificially inflated. They originate from players in Diamond and Master League, which is far from the highest levels of professional play. These outliers are far removed from the median values of their respective leagues. The fact that several values exceed even the highest APM for GrandMaster players suggests that they are not realistically attainable values for players in these leagues. Therefore, I decided to remove data points belonging to these two leagues where *APM* values exceed 275, which is the approximate median value for the professional players represented in this dataset.
...
> With this method, I came to the conclusion that the skill hierarchy proposed by the community is supported by this dataset and should be followed by novice SC2 players that hope to improve.
...
![The 3x3 grid that forms the upper right corner shows how well the three time variables explains each of the three skill variables. The farther from 0, the stronger the relationship is.](correlations.png)


> In the above grid, each square contains a number that ranges from -1 to +1, which indicates the strength of the relationship between two variables. The farther away from 0 the number is, the stronger the relationship is. By looking at the *Age* row, we can see that it contains negative numbers. This provides evidence supporting the belief that a player's movement (as measured by *APM*) is negatively affected by increasing age. *SelectByHotkeys* and *AssignToHotkeys*, skills that have a strong relationship with *APM*, are also negatively affected by incraesing age. On the other hand, the *TotalHours* and *HoursPerWeek* rows contain positive numbers, which hints that these three skills improve the most with additional hours spent playing.


> It should be noted that the relationships visualized above are not proof that there is only one certain way to improve at SC2 or that older players are worse at the game than younger players. They merely provide some evidence that these relationships might exist and justifies further investigation into the matter with a more comprehensive dataset and study.


##5. Main Analysis


> It is commonly thought that higher APM comes naturally through becoming more skilled at SC2. In the SC2 community, it is believed that there is a rough hierarchy of skills that one should acquire in order to become skilled. The variables corresponding to these skills, ranked in approximate descending order of importance, are the following:
...
> At first glance, it seems like there are some correlations between some metrics and the three time variables. More importantly, it looks like all but one of the metrics are negatively correlated with age and positively correlated with playing more hours per week and in total. Interestingly enough, for all three time variables, the three skill variables with the greatest magnitude correlation coefficients are *APM*, *SelectByHotkeys*, and *AssignToHotkeys*. 
...
> It should be noted that there is an obvious correlation between the two hotkey variables and *APM*, which is due to the fact that proper hotkey usage contributes greatly to having a high APM by virtue of allowing faster and more efficient actions, which saves the player time that can be spent on performing more actions. Similarly, this is the case for all of the other variables shown in the correlation matrix as well. Therefore, it is natural that these variables would be correlated in the same direction. 


> One can see that the variance for any given age group decreases with age due to the fact that the sample size for older players is much smaller than for players in their teens and early twenties. Therefore, it looks like there is some evidence that age negatively affects these metrics.
...
> This set of scatterplots shows an even greater positive correlation between the three variables and *TotalHours*. Although these findings might not be completely accurate due to the values being self-reported, I feel that the correlation is strong enough to warrant future research into the relationships among these variables.
...
subset <- data.frame(c(sc[3:5], sc[6:9]))
