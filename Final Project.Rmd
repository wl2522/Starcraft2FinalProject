...
ggplot(sc, aes(sc$League, sc$MinimapAttacks)) + geom_boxplot(fill = 'purple', alpha = 0.5) + theme(panel.background = element_rect(fill = 'white'), panel.grid.major =  element_line(color = 'gray')) + labs(x = 'League', y = 'Minimap Attacks Issued per Minute')


```


> There are a few outlier data points (5/3395) that are obscuring the view of the league distributions. After removing those points, the distribution can be more clearly seen:
...
sc <- filter(sc, sc$MinimapAttacks < 0.0013)
